...
void PHX_camera::initializeId()
{
   //initialize save id
    ...
    CurrentOutputFolder = "/media/nvidia/Data/" + getCurrentDateStr()+"/" + cameraID;
...
*/
            ...
            _updateMutex.lock();      
                ...
                if(ts_img.image_id % 3 ==0 ){ //publish an image for preview every 2s
					...
					cv::imwrite(img_path_preview.c_str(), publishImage);  //write png for view every 10 images
...
int PHXMulti::PHXM_start_capturing(){
    ...
    gpioSetValue(GPIO_PORT, off); //off 8.19
    for(ptrCamera pCamera : cameras_)
    {
...
int PHXMulti::PHXM_trigger(string& ts){  
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(10)); //original 3
